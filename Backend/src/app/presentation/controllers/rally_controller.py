...
@router.get(
    ...
    summary="Get total rally count",
    description="Get the total number of rallies detected in a match. Lightweight endpoint for summary displays."
)
async def get_rally_count(
    match_id: int,
    current_user: Player = Depends(get_current_user),
    rally_service: IRallyService = Depends(get_rally_service)
) -> int:
    ...
    try:
        return await rally_service.get_match_total_rallies(match_id)
        ...
        raise HTTPException(
            ...
            detail={
                "status": "data_unavailable",
                "message": str(e),
                "reason": "The AI could not distinguish individual rallies"
            }
        )
