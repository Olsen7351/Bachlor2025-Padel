...
class Rally:
    ...
    @property
    def shot_count_estimate(self) -> int:
        """
        Estimate shots by counting significant direction changes in ball trajectory.
        """
        ...
        for i in range(len(self.ball_positions)):
            ...
            end = min(len(self.ball_positions), i + window // 2 + 1)
            avg_x = np.mean([p[0] for p in self.ball_positions[start:end]])
            avg_y = np.mean([p[1] for p in self.ball_positions[start:end]])
        ...
        return max(1, (direction_changes + 1) // 2)
...
class RallyTracker:
    ...
    def get_summary(self) -> Dict:
        ...
        shots = [r.shot_count_estimate for r in self.completed_rallies]
        ...
        return {
            ...
            "avg_shots_per_rally": np.mean(shots),
            "max_shots": max(shots),
    ...
    def draw_overlay(self, video_frames: List[np.ndarray]) -> List[np.ndarray]:
        ...
        for frame_idx, frame in enumerate(video_frames):
            ...
            if rally is not None:
                ...
                overlay = frame.copy()
                cv2.rectangle(overlay, (10, 80), (280, 170), (0, 100, 0), -1)
                ...
                cv2.putText(frame, f"Shots: ~{rally.shot_count_estimate}", (20, 165),
    ...
    def export_rallies_csv(self, output_path: str):
        ...
        with open(output_path, 'w', newline='') as f:
            ...
            writer.writerow([
                ...
                'duration_sec', 'estimated_shots', 'ball_positions_count'
            ])
