...
# ============================================================================
# POLYGON EXCLUSION FILTER (Supports Tilted Zones)
# ============================================================================
...
class PolygonExclusionFilter:
    ...
    Unlike rectangular strips, these can follow the angled glass panels
    in perspective view.
    """
    ...
    def add_left_glass_zone(
        ...
        Define 4 corners that trace the visible area of the left adjacent court.
        """
...
# ============================================================================
# COURT INCLUSION FILTER (Alternative approach)
# ============================================================================
...
class CourtInclusionFilter:
    ...
    This is the inverse of exclusion - define where the ball CAN be.
    """
    ...
    def set_court_polygon(self, points: List[Tuple[float, float]]):
        """Set the main court area polygon."""
    ...
    def filter_detection(self, bbox: List[float]) -> bool:
        """Returns True if detection is inside the court."""
...
# ============================================================================
# SPATIAL PLAYER FILTER (No ID dependency)
# ============================================================================
...
class SpatialPlayerFilter:
    ...
    Strategy: 
    - Ball should be within reasonable distance of ANY detected player
    - But we weight players inside the main court area more heavily
    """
    
    def __init__(
        self,
        court_polygon: Optional[np.ndarray] = None,
        max_distance_any_player: float = 500,      # Max distance from any player
        max_distance_court_player: float = 400,    # Stricter for players on court
    ...
    def set_court_polygon(self, points: List[Tuple[float, float]]):
        """Set court polygon for determining which players are 'on court'."""
        self.court_polygon = np.array(points, dtype=np.float32)
    
    def _is_player_on_court(self, bbox: List[float]) -> bool:
        """Check if player is on the main court."""
        if self.court_polygon is None:
            return True
        
        # Use bottom-center of player bbox (their feet)
    ...
    def filter_detection(
        ...
        # If we have court players, use stricter threshold
...
class TrajectoryFilter:
    ...
    def filter_detection(
        ...
        if len(self.velocity_history) >= 2:
            ...
            accel = np.sqrt(
                (current_vel[0] - avg_vel[0])**2 +
                (current_vel[1] - avg_vel[1])**2
            )
...
class BallTrackerTrackNet:
    ...
    def __init__(
        self,
        tracknet_path: str = 'models/TrackNet_best.pt',
        ...
        # Filters - initialize empty, configure via setup methods
        ...
        self.use_inclusion_filter = False  # Off by default (use exclusion instead)
        ...
        # Store confidence for each detection (for display)
    ...
    def _load_tracknet(self) -> TrackNet:
        ...
        checkpoint = torch.load(self.tracknet_path, map_location=self.device)
    ...
    def set_court_polygon(self, points: List[Tuple[float, float]]):
        """
        Set the main court polygon. Used for:
        - Inclusion filter (if enabled)
        - Player filter (to determine which players are 'on court')
        """
        self.inclusion_filter.set_court_polygon(points)
        self.player_filter.set_court_polygon(points)
    
    def setup_glass_panel_exclusions(
        self,
        left_zone: List[Tuple[float, float]],
        right_zone: List[Tuple[float, float]],
    ):
        """
        Convenience method to set up left and right glass panel exclusion zones.
        
        Each zone should be a list of 4+ points defining the polygon where
        the adjacent court is visible through glass.
        """
        ...
        if read_from_stub and stub_path:
            try:
                with open(stub_path, 'rb') as f:
                    data = pickle.load(f)
                    # Handle both old format (just detections) and new format (with confidences)
        ...
        while frame_idx < total_frames:
            ...
            for i in range(actual_batch_size):
                ...
                if self.use_trajectory_filter:
                    accepted, reason = self.trajectory_filter.filter_detection(
                        current_frame_idx, bbox
                    )
                    if not accepted:
                        all_detections.append({})
                        self.detection_confidences.append(None)
    ...
    def draw_bboxes(
        ...
        draw_exclusion_zones: bool = False,
